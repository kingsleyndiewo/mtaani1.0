...
# ---------------------------------------------
class MtaaniTransport(MtaaniNonMorgTile):
    " The base class for all Mtaani transport companies; extends MtaaniTile "
    def __init__(self, name, boardIndex, gameContext):
        MtaaniNonMorgTile.__init__(self, name, boardIndex, gameContext, ['Fare', 'fare', 'transport', 'route',
    ...
    def playerArrives(self, player, boardObj, playerCount):
        # call parent method
        super(MtaaniTransport, self).playerArrives(player, boardObj, playerCount)