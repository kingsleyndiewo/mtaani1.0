...
from MtaaniGlobals import boardTiles, hoods, tycoons
# graphics
import kivy
# version check
kivy.require('1.7.1')
...
# threading
from MtaaniThreads import MtaaniThread
# system utilities
from MtaaniSystem import MtaaniSystem
# ---------------------------------------------
# a class that defines the simple Mtaani board tile
class MtaaniTile(object):
    " The base class for all Mtaani tiles "
    def __init__(self, name, boardIndex, gameContext):
        ...
        self.ownerSpeak = MtaaniInteraction()
    ...
    def createLots(self, boardObj):
        ...
        newLot = MtaaniLot(self, lotColor, [self.lot_width, self.lot_height], [self.lot_w, self.lot_h])
    ...
    def playerArrives(self, player, boardObj, playerCount):
        ...
        if player.loan > 0:
            ...
            if monthsElapsed >= player.ATMTile.loanTerm:
                ...
                self.boardLog.text += "\nMtaani Bank: %s, your outstanding loan is now due" % player.name
    ...
    def placePlayer(self, playerObj, boardObj, playerCount):
        ...
        # use computed widget sizes; the reported ones are inaccurate when window is resized
        widget_w = float(screenW / 13)
        widget_h =  float(screenH / 13)
...
# ---------------------------------------------
# class that defines the simple Mtaani tile lot
class MtaaniLot(object):
    " The base class for all Mtaani tile lots "
